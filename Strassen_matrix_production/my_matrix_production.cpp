...
// global variable for the dimension of the matrix, crossover point
int dimension, crossover;
...
{
    for (int i1{}, i2 = row; i1 != d; i1++, i2++)
    {
        for (int j1{}, j2 = col; j1 != d; j1++, j2++)
        {
            B[i1][j1] = A[i2][j2];
        }
    }
...
void print_matrix(const std::vector<std::vector<int>> &m, int d)
{
    for (int i{}; i != d; i++)
    {
        for (int j{}; j != d; j++)
...
void print_diagonal(std::vector<std::vector<int>> m, int d)
{
    for (int i{}; i != d; i++)
    {
        std::cout << m[i][i] << "\n";
...
{ // ensure proper usage
    ...
    std::vector<int> inside(dimension);
    // initialize matrices to be multiplied
    std::vector<std::vector<int>> A(dimension, inside);
    std::vector<std::vector<int>> B(dimension, inside);
    std::vector<std::vector<int>> C(dimension, inside);
    
    // intialize how many lines we will be scanning
    int size_file = dimension * dimension * 2;
    ...
    if (infile.is_open())
    {
        ...
        for (int i{}, j{}; i != size_file; i++)
        {
            getline(infile, line);
            if (i < (dimension*dimension))
            {
                A[i / dimension][i % dimension] = std::stoi(line);
            }
            else
            {
                B[j / dimension][i % dimension] = std::stoi(line);
                j++;
            }
        }
    }
