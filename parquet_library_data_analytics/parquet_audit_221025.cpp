// /mnt/big/Projects/parquet_readerparquet_audit_new.cpp - previous version
// Lightweight auditor for parquet top/trade/depth files.
// Now can also dump exact rows with id-overlaps/gaps into CSV.
// Build:
//   g++ -std=gnu++23 -O3 parquet_audit_new.cpp -lparquet -larrow -lzstd -o parquet_audit_new
// ./parquet_audit_221025 \
  --out=/mnt/big/Projects/parquet_reader/parquet_audit_report_221025.txt \
  --dump-id-anomalies-dir=/mnt/big/Projects/parquet_reader \
  /mnt/big/Projects/parquet_reader/parquet_files_depth_spot/bn_depth_spot_DFUSDT_2024_4_2.parquet \
  /mnt/big/Projects/parquet_reader/parquet_files_depth_spot/bn_depth_spot_DFUSDT_2024_8_23.parquet
...
static string stem_of(const string &p)
{
...
{
    if (r.non_monotonic_ts > 0)
        return true;
    if (r.lastid_lt_firstid > 0)
        return true;
    if (r.id_overlap_count > 0)
        return true;
    if (r.id_gap_count > 0)
        return true;
    if (r.has_bid_px_but_zero_count > 0)
        return true;
    if (r.has_ask_px_but_zero_count > 0)
        return true;
    if (r.bid_qty_zero > 0)
        return true;
    if (r.ask_qty_zero > 0)
    ...
    if (r.crossed_book_count > 0)
        return true;
    if (r.price_change_10x_count > 0)
        return true;
    if (r.price_not_div1000_count > 0)
        return true;
    if (r.qty_not_div1e8_count > 0)
        return true;
    if (include_info)
    {
        if (r.flattened_without_offsets)
            return true;
        if (r.per_row_offsets_mismatch)
            return true;
    }
    return false;
}
...
// ---------------- NEW: anomaly row capture ----------------

struct IdAnomalyRow
{
    // kind: "OVERLAP" or "GAP"
    string kind;
    // global 1-based row index across the entire file
    uint64_t global_row = 0;
    // scalar fields for quick inspection
    int64_t ts = 0;
    int64_t prev_lastId = 0;
    int64_t firstId = 0;
    int64_t lastId = 0;
    // only for GAP
    int64_t missing = 0;
...
static void write_id_anomalies_csv(const string &out_dir, const string &parquet_path, const vector<IdAnomalyRow> &rows)
{
    if (out_dir.empty() || rows.empty())
    {
        return;
    }
    fs::create_directories(out_dir);

    string base = stem_of(parquet_path);
    string out_csv = (fs::path(out_dir) / (base + string("_id_anomalies.csv"))).string();

    ofstream f(out_csv, ios::trunc);
    if (!f)
    {
        cerr << "ERROR: cannot open CSV for write: " << out_csv << "\n";
        return;
    }
    f << "kind,global_row,ts,prev_lastId,firstId,lastId,missing\n";
    ...
    f.close();
    cerr << "Anomaly CSV written: " << out_csv << " (" << rows.size() << " rows)\n";
}
...
struct AuditOptions
{
    // NEW: optional directory to dump id anomalies
    string dump_id_anomalies_dir;
};

static FileReport audit_parquet_file(const string &path, const AuditOptions &opt, vector<IdAnomalyRow> *out_anoms = nullptr)
{
    FileReport rep;
    rep.file_path = path;
    ...
    if (lfn.find("top") != string::npos)
        rep.type = "top";
    else if (lfn.find("trade") != string::npos)
        rep.type = "trade";
    else if (lfn.find("depth") != string::npos)
        rep.type = "depth";
    else
        rep.type = "unknown";
    ...
    if (firstid_i < 0)
        firstid_i = find_col_idx(schema, "firstid");
    int lastid_i = find_col_idx(schema, "lastId");
    if (lastid_i < 0)
        lastid_i = find_col_idx(schema, "lastid");

    int ask_px_i = find_col_idx(schema, "ask_px");
    if (ask_px_i < 0)
        ask_px_i = find_col_idx(schema, "askpx");
    int ask_qty_i = find_col_idx(schema, "ask_qty");
    if (ask_qty_i < 0)
        ask_qty_i = find_col_idx(schema, "askqty");
    int bid_px_i = find_col_idx(schema, "bid_px");
    if (bid_px_i < 0)
        bid_px_i = find_col_idx(schema, "bidpx");
    int bid_qty_i = find_col_idx(schema, "bid_qty");
    if (bid_qty_i < 0)
        bid_qty_i = find_col_idx(schema, "bidqty");
    ...
    optional<int64_t> prev_price_sample = nullopt; // for 10x change detection
    ...
    for (int rg = 0; rg < md->num_row_groups(); ++rg)
    {
        ...
        if (firstid_i >= 0)
            read_i64_column(*rg_reader, firstid_i, v_firstid);
        if (lastid_i >= 0)
            read_i64_column(*rg_reader, lastid_i, v_lastid);
        ...
        v_bid_qty.clear();
        if (ask_px_i >= 0)
            read_i64_column(*rg_reader, ask_px_i, v_ask_px);
        if (ask_qty_i >= 0)
            read_i64_column(*rg_reader, ask_qty_i, v_ask_qty);
        if (bid_px_i >= 0)
            read_i64_column(*rg_reader, bid_px_i, v_bid_px);
        if (bid_qty_i >= 0)
            read_i64_column(*rg_reader, bid_qty_i, v_bid_qty);

        // read offsets if present (per-row offsets, int32)
        v_ask_off.clear();
        v_bid_off.clear();
        if (ask_off_i >= 0)
            read_i32_column(*rg_reader, ask_off_i, v_ask_off);
        if (bid_off_i >= 0)
            read_i32_column(*rg_reader, bid_off_i, v_bid_off);
        ...
        for (int64_t i = 0; i < rows_in_rg; ++i)
        {
            ...
            if (have_first && have_last)
            {
                if (lastId < firstId)
                {
                    ++rep.lastid_lt_firstid;
                }
                if (prev_lastid.has_value())
                {
                    if (firstId <= *prev_lastid)
                    {
                        ++rep.id_overlap_count;
                        if (out_anoms)
                        {
                            IdAnomalyRow row;
                            row.kind = "OVERLAP";
                            row.global_row = global_rows;
                            row.ts = ts;
                            row.prev_lastId = *prev_lastid;
                            row.firstId = firstId;
                            row.lastId = lastId;
                            row.missing = 0;
                            out_anoms->push_back(row);
                        }
                    }
                    else if (firstId > (*prev_lastid + 1))
                    {
                        ++rep.id_gap_count;
                        if (out_anoms)
                        {
                            ...
                            row.firstId = firstId;
                            row.lastId = lastId;
                            row.missing = firstId - *prev_lastid - 1;
                            out_anoms->push_back(row);
                        }
                    }
                }
            ...
            {
                if (*first_bid_px >= *first_ask_px)
                {
                    ++rep.crossed_book_count;
                }
            }

            // price sample for 10x jump detection
            optional<int64_t> rep_px = first_ask_px.has_value() ? first_ask_px : first_bid_px;
            if (!rep_px.has_value())
            {
                if (ask_px_i >= 0 && static_cast<size_t>(i) < v_ask_px.size())
                    rep_px = v_ask_px[i];
                else if (bid_px_i >= 0 && static_cast<size_t>(i) < v_bid_px.size())
                    rep_px = v_bid_px[i];
            }
            if (rep_px.has_value())
            {
            ...
            {
                if ((*first_bid_px % 1000) != 0)
                    ++rep.price_not_div1000_count;
            }
            if (first_ask_px.has_value())
            {
                if ((*first_ask_px % 1000) != 0)
                    ++rep.price_not_div1000_count;
            }
            if (first_bid_qty.has_value())
            {
                if ((*first_bid_qty % 100000000LL) != 0)
                    ++rep.qty_not_div1e8_count;
            }

            // --- Quantity deviation anomaly check (>1000× from running mean) ---
            {
                optional<int64_t> row_qty;
                if (first_bid_qty.has_value() && *first_bid_qty > 0)
                    row_qty = *first_bid_qty;
                else if (first_ask_qty.has_value() && *first_ask_qty > 0)
                    row_qty = *first_ask_qty;

                static long double running_sum_qty = 0.0L;
                static long double running_mean_qty = 0.0L;
                static uint64_t running_count = 0;

                if (row_qty.has_value())
                {
                    if (running_count > 0 && running_mean_qty > 0.0L)
                    {
                        long double q = static_cast<long double>(*row_qty);
                        long double hi = max(q, running_mean_qty);
                        long double lo = min(q, running_mean_qty);
                        if (hi / lo > 1000.0L)
                        {
                            ++rep.qty_extreme_deviation_count;
                        }
                    }
                    // update running mean
                    running_sum_qty += static_cast<long double>(*row_qty);
                    ++running_count;
                    running_mean_qty = running_sum_qty / running_count;
                }
            }

            if (first_ask_qty.has_value())
            {
                if ((*first_ask_qty % 100000000LL) != 0)
                    ++rep.qty_not_div1e8_count;
            }
        } // per-row loop

        // element-level zero counts
        for (int64_t v : v_bid_qty)
            if (v == 0)
                ++rep.bid_qty_zero;
        for (int64_t v : v_ask_qty)
            if (v == 0)
                ++rep.ask_qty_zero;
        for (int64_t v : v_bid_px)
            if (v == 0)
                ++rep.bid_px_zero;
        for (int64_t v : v_ask_px)
            if (v == 0)
                ++rep.ask_px_zero;
    } // row-groups

    // if file declares bid/ask arrays but element counts are zero -> suspicious
    if ((bid_px_i >= 0 || bid_qty_i >= 0) && rep.bid_px_count == 0)
        rep.has_bid_px_but_zero_count = rep.rows_scanned;
    if ((ask_px_i >= 0 || ask_qty_i >= 0) && rep.ask_px_count == 0)
        rep.has_ask_px_but_zero_count = rep.rows_scanned;

    // NEW: write anomalies CSV if requested
    if (!opt.dump_id_anomalies_dir.empty() && out_anoms && !out_anoms->empty())
    {
        write_id_anomalies_csv(opt.dump_id_anomalies_dir, path, *out_anoms);
    }
...
static void write_report_filtered(const string &outpath, const vector<FileReport> &reports, bool include_info)
{
    ...
    {
        if (!is_problematic(r, include_info))
            continue;
        ++problems;
        f << "File: " << r.file_path << "\n";
        f << "Type: " << r.type << "\n";
        ...
        f << "  qty_extreme_deviation_count: " << r.qty_extreme_deviation_count << "\n";
        ...
        f << "\nDetailed notes:\n";
        if (r.non_monotonic_ts > 0)
            f << "  -> Non-monotonic timestamps found: timestamps decreased.\n";
        if (r.lastid_lt_firstid > 0)
            f << "  -> lastId < firstId inside some rows.\n";
        if (r.id_overlap_count > 0)
            f << "  -> ID range overlaps detected (firstId <= prev.lastId).\n";
        if (r.id_gap_count > 0)
            f << "  -> ID gaps detected (missing delta segments between rows).\n";
        if (r.bid_px_zero > 0 || r.ask_px_zero > 0)
            f << "  -> Some price elements are zero (likely invalid).\n";
        if (r.bid_qty_zero > 0 || r.ask_qty_zero > 0)
            f << "  -> Some quantity elements are zero (suspicious).\n";
        if (r.crossed_book_count > 0)
            f << "  -> Crossed book rows (best_bid >= best_ask).\n";
        if (r.price_change_10x_count > 0)
            f << "  -> Price changed more than 10x between adjacent samples.\n";
        if (r.flattened_without_offsets)
            f << "  -> Arrays present but no offsets found: cannot map elements to rows precisely.\n";
        if (r.qty_extreme_deviation_count > 0)
            f << "  -> Quantity deviated from running mean by more than 1000× in some rows.\n";
        if (r.per_row_offsets_mismatch)
            f << "  -> Offsets array length does not match rows+1 (per-RG) in at least one RG.\n";
...
int main(int argc, char **argv)
{
    if (argc < 2)
    {
        cerr << "Usage: " << argv[0] << " <parquet-file-1> [<parquet-file-2> ...] [--out=report.txt] [--include-info] [--dump-id-anomalies-dir=/path]\n";
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (a.rfind("--dump-id-anomalies-dir=", 0) == 0) // NEW
        {
            aopt.dump_id_anomalies_dir = a.substr(strlen("--dump-id-anomalies-dir="));
        }
        else
        {
            files.push_back(a);
    ...
    for (const string &f : files)
    {
        ...
        // Collect anomalies per file if dumping is requested
        vector<IdAnomalyRow> anoms;

        try
        {
            FileReport r = audit_parquet_file(f, aopt, aopt.dump_id_anomalies_dir.empty() ? nullptr : &anoms);
        ...
        // If the option is set, CSV is written inside audit_parquet_file() already
        (void)anoms;
    }
